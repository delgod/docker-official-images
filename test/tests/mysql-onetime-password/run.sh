...
if docker logs "$cname" 2>&1 | grep 'Sorry, this version of MySQL does not support "PASSWORD EXPIRE"'; then
	exit 0
fi
...
echo "SELECT 1" \
	...
	"mysql" \
	2>&1 || : ) \
	| egrep -q '(Please use --connect-expired-password|ERROR 1862 [(]HY000[)]: Your password has expired.)'
